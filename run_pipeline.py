...
if __name__ == "__main__":
    ...
    parser.add_argument("--rerun_preds", action="store_true", help="(Optional) Rerun predictions if they already exist")
    parser.add_argument("--rerun_eval", action="store_true", help="(Optional) Rerun eval if they already exist")
    parser.add_argument("--skip_mutation", action="store_true", help="(Optional) Skip mutation")
    parser.add_argument("--azure", action="store_true", help="(Optional) Run with azure")
    parser.add_argument("--skip_full", action="store_true", help="(Optional) Skip full inference")

    args = parser.parse_args()

    print("NOTE: Make sure you have built the docker images for the appropriate dataset")
    ...
    print(f"Running pipeline for {args.model} with pass@{args.num_samples} on {data_suf}")
    ...
    API_MODELS = ["gpt-4o-2024-05-13", "gpt-4-0613", "gpt-4-turbo-2024-04-09", "gpt-3.5-turbo-0125", "Meta-Llama-3.1-405B-Instruct"]
    if not os.path.exists(preds_file):
        if args.model in API_MODELS:
            ...
            model_cmd = ["python", "-m", "inference.api.run_api", "--model_name_or_path", args.model, "--dataset_name_or_path", dataset_dir, "--output_dir", pred_dir, "--num_samples", str(args.num_samples)] + model_extra_cmd
            subprocess.run(model_cmd)
        else:
            model_cmd = ["python", "-m", "inference.huggingface.run_huggingface", "--model_name_or_path", args.model, "--dataset_name_or_path", dataset_dir, "--use_auth_token", "--output_dir", pred_dir, "--num_samples_completion", str(args.num_samples), "--temperature", str(args.temperature)] 
            subprocess.run(model_cmd)

    # Run evaluation
    extra_cmd = ["--skip_existing"] if not args.rerun_eval else []
    extra_cmd += ["--skip_mutation"] if args.skip_mutation and args.model != "baseline" else []
    if args.model == "baseline":  
        eval_cmd = ["python", "run_evaluation_baseline.py", "--log_dir", log_dir, "--swe_bench_tasks", dataset_dir, "--num_processes", str(args.num_processes)] + extra_cmd
        report_cmd = ["python", "generate_report_baseline.py", "--log_dir", log_dir, "--output_dir", base_dir, "--swe_bench_tasks", dataset_dir]
    else:
        eval_cmd = ["python", "run_evaluation.py", "--predictions_path", preds_file, "--log_dir", log_dir, "--swe_bench_tasks", dataset_dir, "--num_processes", str(args.num_processes)] + extra_cmd
        report_cmd = ["python", "generate_report.py", "--predictions_path", preds_file, "--log_dir", log_dir, "--output_dir", base_dir, "--swe_bench_tasks", dataset_dir]
