            ...
            with torch.no_grad():
                for step, batch in tqdm(enumerate(dataloader), total=total):
                    ...
                    for i in range(num_samples):
                        ...
                        outputs = outputs[0][batch["input_len"]:]  # Skip the input tokens
                        generated_texts += [tokenizer.decode(outputs, skip_special_tokens=True)]
                    
                    generated_instance_ids = np.array(batch["instance_id"]).repeat(num_samples).tolist()
                    generated_ids = np.array(batch["id"]).repeat(num_samples).tolist()
                    generated_settings = np.array(batch["setting"]).repeat(num_samples).tolist()
                    prompts = np.array(batch["prompt"]).repeat(num_samples).tolist()

                    # Write prompts to the separate prompt file
                    for prompt, setting, curr_id in zip(prompts, generated_settings, generated_ids):
                    ...
                    for instance_id, curr_id, setting, text in zip(generated_instance_ids, generated_ids, generated_settings, generated_texts):
                        text_processed = postprocess_fn(text, is_full)
                        pred_f.write(json.dumps({"instance_id": instance_id, "id": curr_id, "setting": setting, "pred": text_processed}) + "\n")
