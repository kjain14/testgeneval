...
MODEL_CONFIG = {
    "CodeLlama-7b-Instruct-hf": {"tensor_parallel_size": 8, "max_context_window": 16_384, "prompt_class": InstructPrompt, "eager": True, "huggingface": False},
    "CodeLlama-70b-Instruct-hf": {"tensor_parallel_size": 8, "max_context_window": 4_096, "prompt_class": InstructPrompt, "eager": True, "huggingface": False},
    "Meta-Llama-3-8B-Instruct": {"tensor_parallel_size": 8, "max_context_window": 8_192, "prompt_class": Llama3Prompt, "eager": True, "huggingface": False},
    "Meta-Llama-3-70B-Instruct": {"tensor_parallel_size": 8, "max_context_window": 8_192, "prompt_class": Llama3Prompt, "eager": True, "huggingface": False},
    "Meta-Llama-3.1-8B-Instruct": {"tensor_parallel_size": 8, "max_context_window": 128_000, "prompt_class": Llama3Prompt, "eager": True, "huggingface": False},
    "Meta-Llama-3.1-70B-Instruct": {"tensor_parallel_size": 8, "max_context_window": 128_000, "prompt_class": Llama3Prompt, "eager": True, "huggingface": False},
    "DeepSeek-Coder-V2-Lite-Instruct": {"tensor_parallel_size": 8, "max_context_window": 128_000, "prompt_class": InstructPrompt, "eager": True, "huggingface": False},
    "gemma-2-9b-it": {"tensor_parallel_size": 8, "max_context_window": 8_192, "prompt_class": Gemma2Prompt, "eager": True,  "huggingface": True},
    "gemma-2-27b-it": {"tensor_parallel_size": 8, "max_context_window": 8_192, "prompt_class": Gemma2Prompt, "eager": True,  "huggingface": True},
    "Codestral-22B-v0.1": {"tensor_parallel_size": 8, "max_context_window": 32_000, "prompt_class": CodestralPrompt, "eager": True,  "huggingface": False},
    "Meta-Llama-3.1-405B-Instruct": {"tensor_parallel_size": 16, "max_context_window": 128_000, "prompt_class": Llama3Prompt, "eager": True, "huggingface": False},
}
...
def parse_args():
    ...
    parser.add_argument(
        "--local_model_name",
        type=str,
        help="Name of local model.",
        default=""
    )

    parser.add_argument(
        "--skip_full",
        help="Skip full setting",
        action="store_true"
    )
    ...
    for datum in dataset:
        for prompt_name, prompt in datum["preds_prompts"].items():
            if datum["id"] in existing_ids and prompt_name in existing_ids[datum["id"]]:
                continue
            if prompt_name == "full":
                new_ds_full.append({"instance_id": datum["instance_id"], "id": datum["id"], "setting": prompt_name, "prompt": prompt})
            else:
                new_ds_completion.append({"instance_id": datum["instance_id"], "id": datum["id"], "setting": prompt_name, "prompt": prompt})
...
def main():
    ...
    model_config = MODEL_CONFIG.get(model_nickname) if len(args.local_model_name) == 0 else MODEL_CONFIG.get(args.local_model_name)
    ...
    stop_token_ids = None if "Llama-3" not in model_nickname else [tokenizer.eos_token_id, tokenizer.convert_tokens_to_ids("<|eot_id|>")]
    ...
    dataset = prompt_info.add_prompts_to_dataset(dataset, tokenizer=tokenizer, no_import=args.no_imports)["test"]
                ...
                for line in f:
                    ...
                    for setting in data["preds"]:
                        for pred in data["preds"][setting][:5]:
                            lines_full.append({"instance_id": instance_id, "id": data_id, "setting": setting, "prompt": data["preds_prompts"][setting], "pred": pred})
    ...
    if len(dataset) > 0:
        ...
        generator = Generator(model, tokenizer, args.temperature, output_file_raw, output_file_prompt, use_huggingface=huggingface)

        if len(dataset_full) > 0 and not args.skip_full:
            dataset_full = truncate_prompts(dataset_full, tokenizer, max_tokens)
            generator.generate(dataset_full, prompt_info.postprocess_output, True, args.num_samples_generation, 8192, stop_token_ids)
        if len(dataset_completion) > 0 and not args.skip_completion:
            dataset_completion = truncate_prompts(dataset_completion, tokenizer, max_tokens)
            generator.generate(dataset_completion, prompt_info.postprocess_output, False, args.num_samples_completion, 512, stop_token_ids)

        process_raw_output(output_file_raw, output_file_prompt, output_file, model_nickname + f"t={args.temperature}")
